...
edit_csvfile() {
	...
	read -rep "Enter the entry number (space-separated) : " entry_number_var && entry_number=($(echo "$entry_number_var" | tr ' ' '\n' | sort -u)) # only take uniques
	...
	for entry_number1 in "${entry_number[@]}" ; do
		...
		read -rep "Enter the new comment: " new_comment
...
#--------------------------------------------------------------------- READ INPUT ------------------------------------------------------------------------------------------------------#
	while true; do
		...
		read -rep "Enter subvolume name (space-separated) : " subvolume_name_var && subvolume_name=($(echo "$subvolume_name_var" | tr ' ' '\n' | sort -u)) # only take uniques
	...
	read -rep "Enter comment for snapshot: "                comment
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
delete_snapshot() {
	...
	read -rep "Enter Snapshot number to Delete (space-separated) : " del_snap_var && del_snap=($(echo "$del_snap_var" | tr ' ' '\n' | sort -u)) # only take uniques
...
rollback_snapshot() {
	...
	while true; do
		...
		read -rep "Enter Snapshot number to rollback : "  rollback_number #enter only one digit
...
while true; do
	...
	read -rep "Enter Snapshot subvolume to rollback (space-separated) : " rollback_subvol_var && rollback_subvol=($(echo "$rollback_subvol_var" | tr ' ' '\n' | sort -u)) # only take uniques
...
# read -rep "Enter Snapshot name to Delete Seprate it by space : " -a lines
